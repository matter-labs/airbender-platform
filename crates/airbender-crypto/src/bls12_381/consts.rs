...
// The prepared form of `G2_BY_TAU_POINT` (its Miller-loop line coefficients), generated by
// `prepared_tau_generator::print_prepared_g2_by_tau` below, so that the KZG point evaluation does
// not recompute it for every proof.
#[cfg(not(any(
    all(target_arch = "riscv32", feature = "bigint_ops"),
    feature = "proving",
    test
...
#[cfg(test)]
mod prepared_tau_generator {
    //! Prints the Rust source of `PREPARED_G2_BY_TAU` in both representations; run with
    //! `cargo test -p airbender-crypto --lib print_prepared_g2_by_tau -- --nocapture --ignored`
    use crate::bls12_381::consts::G2_BY_TAU_POINT;
    use crate::bls12_381::curves::G2PreparedNoAlloc;
    use ark_ff::PrimeField;

    fn limbs_ref(x: ark_bls12_381::Fq) -> [u64; 6] {
        x.0 .0
    }

    #[test]
    #[ignore]
    fn print_prepared_g2_by_tau() {
        let conv = |x: crate::bls12_381::Fq| {
            let mut limbs = [0u64; 6];
            limbs.copy_from_slice(&x.into_bigint().0[..6]);
            ark_bls12_381::Fq::from_bigint(ark_ff::BigInt(limbs)).unwrap()
        };
        let tau = G2_BY_TAU_POINT;
        let tau_ref = ark_bls12_381::G2Affine::new_unchecked(
            ark_bls12_381::Fq2::new(conv(tau.x.c0), conv(tau.x.c1)),
            ark_bls12_381::Fq2::new(conv(tau.y.c0), conv(tau.y.c1)),
        );
        assert!(tau_ref.is_on_curve() && tau_ref.is_in_correct_subgroup_assuming_on_curve());
        let host = ark_ec::bls12::G2Prepared::<ark_bls12_381::Config>::from(tau_ref);
        let guest = G2PreparedNoAlloc::from(tau);
        assert_eq!(host.ell_coeffs.len(), guest.ell_coeffs.len());
        let mut out = String::new();
        let mut emit = |cfg: &str, fp_path: &str, rows: Vec<[Vec<u64>; 6]>| {
            out.push_str(cfg);
            out.push_str("pub const PREPARED_G2_BY_TAU:\n    <crate::bls12_381::curves::Bls12_381 as crate::ark_ec::pairing::Pairing>::G2Prepared =\n    crate::bls12_381::curves::G2PreparedNoAlloc {\n        ell_coeffs: [\n");
            for row in rows {
                out.push_str("            (\n");
                for pair in row.chunks(2) {
                    out.push_str("                crate::ark_ff::fields::models::Fp2 {\n");
                    for (name, limbs) in ["c0", "c1"].iter().zip(pair) {
                        out.push_str(&format!("                    {name}: {fp_path}(\n                        crate::BigInt({limbs:?}),\n                        core::marker::PhantomData,\n                    ),\n"));
                    }
                    out.push_str("                },\n");
                }
                out.push_str("            ),\n");
        ...
        let host_rows: Vec<[Vec<u64>; 6]> = host
            .ell_coeffs
            .iter()
            .map(|(a, b, c)| [a.c0, a.c1, b.c0, b.c1, c.c0, c.c1].map(|x| limbs_ref(x).to_vec()))
            .collect();
        let guest_rows: Vec<[Vec<u64>; 6]> = guest
            .ell_coeffs
            .iter()
            .map(|(a, b, c)| [a.c0, a.c1, b.c0, b.c1, c.c0, c.c1].map(|x| x.0 .0.to_vec()))
            .collect();
        emit(
            "#[cfg(not(any(\n    all(target_arch = \"riscv32\", feature = \"bigint_ops\"),\n    feature = \"proving\",\n    test\n)))]\n",
            "crate::ark_ff::fields::models::Fp",
            host_rows,
        );
        emit(
            "#[cfg(any(\n    all(target_arch = \"riscv32\", feature = \"bigint_ops\"),\n    feature = \"proving\",\n    test\n))]\n",
            "crate::ark_ff_delegation::Fp",
            guest_rows,
        );
        std::fs::write("/tmp/prepared_g2_by_tau.rs", out).unwrap();
        // both forms encode the same coefficients
        for ((a, b, c), (ra, rb, rc)) in guest.ell_coeffs.iter().zip(host.ell_coeffs.iter()) {
            for (x, rx) in [a.c0, a.c1, b.c0, b.c1, c.c0, c.c1]
                .iter()
                .zip([ra.c0, ra.c1, rb.c0, rb.c1, rc.c0, rc.c1].iter())
            {
                assert_eq!(conv(*x), *rx);
            }
        }
    ...
    /// The constant is what the preparation computes
    #[test]
    fn prepared_g2_by_tau_matches_preparation() {
        assert_eq!(
            crate::bls12_381::consts::PREPARED_G2_BY_TAU,
            G2PreparedNoAlloc::from(G2_BY_TAU_POINT)
        );
    }
}
