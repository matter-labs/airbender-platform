...
/// Host prover interface used by `Program::prove`.
pub trait Prover {
...
pub(super) fn ensure_prover_program_match(
    expected_app_bin_path: &Path,
    provided_app_bin_path: &Path,
) -> Result<()> {
    let provided_app_bin_path = resolve_app_bin_path(provided_app_bin_path)?;

    if expected_app_bin_path != provided_app_bin_path {
        return Err(HostError::Prover(format!(
            "prover is bound to {}, but got request for {}",
            expected_app_bin_path.display(),
            provided_app_bin_path.display()
        )));
    }
...
pub(super) fn receipt_from_proof(proof: &UnrolledProgramProof) -> Receipt {
