...
pub struct ResolvedBuildParams {
    ...
    /// Target triple, or `None` to use the toolchain default.
...
impl BuildConfig {
    ...
    pub fn resolve(&self, cwd: &Path) -> Result<ResolvedBuildParams> {
        validate_app_name(&self.app_name)?;

        let project_dir = self.resolve_project_dir(cwd);
        ...
        let git = resolve_git_metadata(&project_dir);

        Ok(ResolvedBuildParams {
            project_dir,
    ...
    fn resolve_dist_dir(
        ...
        let dist_root = self.dist_dir.as_ref().map_or_else(
...
fn resolve_git_metadata(project_dir: &Path) -> GitMetadata {
...
mod tests {
    ...
    fn resolve_defaults() {
        ...
        assert_eq!(params.dist_dir, dir.path().join("dist/app"));
    }
    ...
    fn resolve_panic_immediate_abort_reads_manifest() {
        ...
        let params = config.resolve(&project_dir).expect("resolve");
        assert!(!params.panic_immediate_abort);
    }
    ...
    fn resolve_propagates_explicit_target() {
        ...
        let params = config.resolve(dir.path()).expect("resolve");
        assert_eq!(params.target.as_deref(), Some("riscv32im-risc0-zkvm-elf"));
    }

    #[test]
    fn resolves_relative_project_dir_from_invocation_cwd() {
        let config = BuildConfig::new("examples/fibonacci/guest");
        let project_dir = config.resolve_project_dir(Path::new("/workspace/repo"));
        assert_eq!(
            project_dir,
    ...
    fn resolves_absolute_project_dir_without_rebasing() {
        ...
        let project_dir = config.resolve_project_dir(Path::new("/other/cwd"));
    ...
    fn validate_app_name_accepts_simple_name() {
        assert!(validate_app_name("app").is_ok());
        assert!(validate_app_name("my-app").is_ok());
        assert!(validate_app_name("my_app_123").is_ok());
    }
    ...
    fn validate_app_name_rejects_slash() {
        let err = validate_app_name("foo/bar").expect_err("slash must fail");
        assert!(matches!(err, crate::BuildError::InvalidConfig(_)));
    }

    #[test]
    fn validate_app_name_rejects_backslash() {
        let err = validate_app_name("foo\\bar").expect_err("backslash must fail");
        assert!(matches!(err, crate::BuildError::InvalidConfig(_)));
    }

    #[test]
    fn validate_app_name_rejects_dotdot() {
        let err = validate_app_name("..").expect_err("dotdot must fail");
        assert!(matches!(err, crate::BuildError::InvalidConfig(_)));
    }

    // --- resolve_bin_name ---

    fn make_manifest(package_name: &str, bin_targets: &[&str]) -> ProjectManifest {
        ProjectManifest {
    ...
    fn resolve_bin_name_uses_single_candidate_without_explicit_override() {
        let resolved = resolve_bin_name(&make_manifest("guest", &["guest"]), None)
    ...
    fn resolve_bin_name_rejects_ambiguous_multi_bin_without_explicit_override() {
        let err = resolve_bin_name(&make_manifest("guest", &["alpha", "beta"]), None)
    ...
    fn resolve_bin_name_validates_explicit_bin_against_candidates() {
        let err = resolve_bin_name(&make_manifest("guest", &["alpha", "beta"]), Some("gamma"))
    ...
    fn resolve_bin_name_accepts_explicit_candidate() {
        let resolved = resolve_bin_name(&make_manifest("guest", &["alpha", "beta"]), Some("beta"))
    ...
    fn resolve_bin_name_rejects_packages_without_binaries() {
        let err = resolve_bin_name(&make_manifest("guest", &[]), None)
