...
Start reth in dev mode and generate some non-empty blocks:

```sh
cd docker
bash generate-blocks.sh
```
...
Witness: 12 state nodes, 2 codes, 3 keys, 1 headers
Recovered 1 public keys
Expected block hash: 0x0964...bc76
Simulation verified: block hash matches.
Proof verified: block 2 (hash=0x0964...bc76) proven in ZK.
```
...
2. Fetches the raw block (`debug_getRawBlock`) and execution witness (`debug_executionWitness`).
3. Recovers transaction signer public keys from ECDSA signatures.
4. Constructs a dev chain config (chain_id=1337, all forks including Prague at genesis).
...
This example patches `revm-interpreter` 32.0.0 with `#[inline(never)]` on `memory_gas`
(`src/gas.rs`), preventing LLVM from combining inlined calls into a `mulh`-producing pattern.
The patch is at [Jrigada/revm-interpreter-patched](https://github.com/Jrigada/revm-interpreter-patched)
and applied via `[patch.crates-io]` in `guest/Cargo.toml`.
